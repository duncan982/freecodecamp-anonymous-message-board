      ...
      chai
        ...
        .end((err, res) => {
          if (err) {
            ...
            assert.property(res.body, 'board', 'Board parameter should be set in the URL');
            assert.property(res.body, 'text', 'Text should be sent in the body');
            assert.property(res.body, 'delete_password', 'Password to delete should be sent in the body');
      ...
      chai
        ...
        .end((err, res) => {
          if (err) {
            ...
            assert.equal(res.status, 200);
            assert.isArray(res.body, 'Gotten value should be an array (of objects)')
            let tlength = res.body.length;
            assert.isAtMost(tlength, 10, 'Length of returned array should not be more than 10');
            for (let i of res.body) {
              assert.property(i, 'replies', 'Object should contain the property replies')
              assert.isArray(i.replies, 'the replies value should be an array')
              let rlength = i.replies.length
              assert.isAtMost(rlength, 3, 'Length of the replies value array should not be more than 3');
            }
            done();
          }
        });
    ...
    test('Test POST /api/replies/:board by creating a new reply', (done) => {
      chai
        ...
        .end((err, res) => {
          if (err) done(err);
          assert.equal(res.status, 200);
          assert.property(res.body, '_id', 'Response should contain a thread_id property');
          assert.property(res.body, 'delete_password', 'Response should contain a deleted_password property');
          assert.property(res.body, 'reported', 'Response should contain a reported property');
          assert.property(res.body, 'text', 'Response should contain a text property');
          assert.property(res.body, 'thread_id', 'Response should contain a thread_id property');
          assert.equal(res.body.text, 'test reply text', 'value of text property should equal "test reply text"')
    ...
    test('Test GET /api/replies/:board by getting all replies to a thread', (done) => {
      chai
        ...
        .end((err, res) => {
          ...
          assert.property(res.body, 'text', 'Response should contain a text property');
          assert.property(res.body, 'created_on', 'Response should contain a created_on property');
          assert.property(res.body, 'replies', 'Response should contain a replies property');
          assert.property(res.body, 'replycount', 'Response should contain a replycount property');
          assert.isArray(res.body.replies, 'Value of replies property should be an array');
          assert.isNumber(res.body.replycount, 'Value of replycount property should be a number');
